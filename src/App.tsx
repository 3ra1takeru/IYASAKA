...
interface ServiceDetailModalProps {
    ...
    onApplyService: (service: Service) => void;
}
...
const ServiceDetailModal: React.FC<ServiceDetailModalProps> = ({ isOpen, onClose, service, provider, currentUser, onRequestService, reviews, serviceOrders, users, onAddReview, onApplyService }) => {
    ...
    const canRequest = currentUser && currentUser.id !== service.providerId;
                    ...
                    {canRequest ? (
                        <div className="flex flex-col gap-3">
                            ...
                            <button
                                ...
                                className="w-full bg-indigo-600 text-white px-4 py-3 rounded hover:bg-indigo-700 transition-colors flex items-center justify-center font-bold text-lg tracking-wide shadow-sm"
                            >
                                <span className="mr-2">✨</span>
                                このサービスに申し込む (¥{service.price.toLocaleString()})
                            </button>
                        </div>
                    ) : (
                        ...
                        return (
                            <div key={reg.id} className="p-4 border border-stone-200 rounded-lg bg-white shadow-sm">
                                <div className="flex justify-between items-start mb-2">
                                    ...
                                    <span className={`px-2 py-1 text-xs font-bold rounded ${reg.status === RegistrationStatus.APPROVED ? 'bg-green-100 text-green-800' :
...
const App: React.FC = () => {
    ...
    const [reservationTarget, setReservationTarget] = useState<{ type: 'event_ticket' | 'exhibitor_fee' | 'service', targetId: number | string, title: string, amount: number, providerStripeAccountId?: string } | null>(null);

    const handleOpenReservation = (type: 'event_ticket' | 'exhibitor_fee' | 'service', targetId: number | string, title: string, amount: number, providerStripeAccountId?: string) => {
        setReservationTarget({ type, targetId, title, amount, providerStripeAccountId });
    ...
    const handleAddService = (newServiceData: Omit<Service, 'id' | 'providerId' | 'status'>) => {
        if (!currentUser || (currentUser.role !== UserRole.PROVIDER && currentUser.role !== UserRole.ADMIN)) return;
    ...
    const handleMemberReserve = (eventId: string) => {
        if (!currentUser) {
            showMessage("ログインが必要", "イベントに参加するにはログインしてください。");
            return;
        }
        ...
        // Use configured admission fee or default if required
        const price = event.isAdmissionFeeRequired
            ? (event.advanceTicketPrice || 1000)
            : 0;

        // If free, maybe just auto-confirm? For now keep flow consistent with others even if 0 (or handle free logic later)
        // User asked for admission fee handling, so assuming paid.

        handleOpenReservation('event_ticket', eventId, event.name, price);
    };
    ...
    const handleBookTimeSlot = (eventId: string, providerId: string, timeSlotId: string, bookingType: 'online' | '現地') => {
        if (!currentUser || (currentUser.role !== UserRole.MEMBER && currentUser.role !== UserRole.PROVIDER)) return;
    ...
    const handleApplyService = (service: Service) => {
        if (!currentUser || (currentUser.role !== UserRole.MEMBER && currentUser.role !== UserRole.PROVIDER)) {
            showMessage("ログインが必要", "サービスに申し込むには、ログインしてください。");
            return;
        }

        // Find provider to get Stripe Account ID (In real app, this field would exist on Provider record)
        // const provider = providers.find(p => p.id === service.providerId);
        // const stripeAccountId = provider?.stripeAccountId; 

        // For demo, we don't have real stripe IDs, so we pass undefined or a mock if we had one.
        handleOpenReservation('service', service.id, service.title, service.price, undefined);
    };
    const handleRequestService = (serviceId: string) => {
        if (!currentUser || (currentUser.role !== UserRole.MEMBER && currentUser.role !== UserRole.PROVIDER)) {
            showMessage("ログインが必要です", "サービスをリクエストするにはログインしてください。");
            return;
        }
    ...
    const handleLinkStripe = async () => {
        if (!currentUser) return;
        try {
            showMessage("処理中", "Stripe連携ページへ移動します...");
            let accountId = currentUser.stripeAccountId;

            if (!accountId) {
                const res = await api.createConnectAccount(currentUser.id, `user_${currentUser.id}@example.com`);
                accountId = res.accountId;
                setCurrentUser(prev => prev ? { ...prev, stripeAccountId: accountId } : null);
                setUsers(prev => prev.map(u => u.id === currentUser.id ? { ...u, stripeAccountId: accountId } : u));
            }
            const linkRes = await api.createAccountLink(accountId!);
            window.location.href = linkRes.url;
        } catch (error: any) {
            console.error('Stripe Link Error:', error);
            showMessage("エラー", `Stripe連携に失敗しました。\n${error.message || '不明なエラー'}`);
        }
    };
            ...
            <main className="flex-grow container mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="space-y-6">
                        ...
                        {/* Stripe Connect Settings (Provider Only) */}
                        {(currentUser.role === UserRole.PROVIDER || currentUser.role === UserRole.ADMIN) && (
                            <div className="bg-white rounded shadow-sm p-6 border border-stone-200 mt-6">
                                <h3 className="font-semibold text-stone-700 mb-2 text-sm">売上受取設定 (Stripe)</h3>
                                {currentUser.stripeAccountId ? (
                                    <div>
                                        <p className="text-green-600 text-sm flex items-center mb-2"><span className="mr-2">●</span>連携済み</p>
                                        <button onClick={handleLinkStripe} className="text-xs text-stone-500 underline hover:text-stone-700">設定を確認・変更する</button>
                                    </div>
                                ) : (
                                    <div>
                                        <p className="text-stone-500 text-xs mb-2">売上を受け取るための口座情報を登録します。</p>
                                        <button onClick={handleLinkStripe} className="w-full bg-[#635BFF] text-white py-2 rounded text-sm font-bold hover:bg-[#5249e0] transition-colors">受取口座を設定する</button>
                                    </div>
                                )}
                            </div>
                        )}
        ...
        <div className="min-h-screen flex flex-col font-serif bg-stone-50/50">
            ...
            {reservationTarget && (
                <ReservationModal
                    ...
                    type={reservationTarget.type}
                    targetId={typeof reservationTarget.targetId === 'string' ? 0 : reservationTarget.targetId} // Mock ID conversion for demo
                    title={reservationTarget.title}
                    amount={reservationTarget.amount}
                    providerStripeAccountId={reservationTarget.providerStripeAccountId}
                    currentUser={currentUser}
                />
            )}
        </div>
