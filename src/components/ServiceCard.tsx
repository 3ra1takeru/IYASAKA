...
const ServiceCard: React.FC<ServiceCardProps> = ({ service, provider, onSelect }) => {
  ...
  return (
    <div className="bg-white border border-stone-200 rounded shadow-sm hover:shadow-md overflow-hidden transform hover:-translate-y-0.5 transition-all duration-300 flex flex-col text-left w-full group">
      ...
      <div className="p-6 flex flex-col flex-grow">
        ...
        <div className="flex items-center text-sm text-stone-600 mt-auto pt-4 border-t border-stone-100 w-full">
          ...
          <div className="ml-auto flex items-center space-x-3">
            <span className="text-lg font-bold text-teal-800 font-serif">&yen;{service.price.toLocaleString()}</span>
            <button
              onClick={(e) => {
                e.stopPropagation();
                onSelect(service);
              }}
              className="bg-teal-700 text-white text-xs px-3 py-1.5 rounded-full hover:bg-teal-800 transition-colors shadow-sm"
            >
              申し込む
            </button>
          </div>
